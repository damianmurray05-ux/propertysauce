/* Property Sauce assistant.
   Tenants: verify with tenancy reference + one-time code, then talk to the
   AI to report a repair. Prospective tenants: discuss tenancy terms.
   Everyone else: a guided enquiry. If the API is not reachable (static
   hosting without the functions deployed) it degrades to phone and email. */
(() => {
  ...
  const PHONE = "+44 (0)20 8988 8434", PHONE_HREF = "tel:+442089888434", EMAIL = "contact@propertysauce.co";
  const API = { chat: "/api/chat/", verify: "/api/verify/" };

  const state = load() || { mode: null, session: null, tenant: null, history: [], transcript: [] };
  let opened = false;

  function save() { try { sessionStorage.setItem("ps-assistant", JSON.stringify({ ...state, history: state.history.slice(-40) })); } catch {} }
  function load() { try { return JSON.parse(sessionStorage.getItem("ps-assistant")); } catch { return null; } }
  ...
  const ask = (placeholder, next, opts = {}) => {
    ...
    f.innerHTML = `<label class="sr-only" for="chat-in">${placeholder}</label>${multi ? `<textarea id="chat-in" rows="2" placeholder="${placeholder}"></textarea>` : `<input id="chat-in" type="${opts.type || "text"}" inputmode="${opts.inputmode || "text"}" autocomplete="${opts.autocomplete || "off"}" placeholder="${placeholder}" required>`}${opts.photos ? `<label class="btn btn-ghost btn-sm" title="Add a photo"><span class="sr-only">Add a photo</span><svg class="ic" aria-hidden="true"><use href="/assets/icons.svg#camera"/></svg><input type="file" accept="image/*" multiple hidden></label>` : ""}<button class="btn btn-primary btn-sm" type="submit" aria-label="Send"><svg class="ic" aria-hidden="true"><use href="/assets/icons.svg#arrow-up-right"/></svg></button>`;
  ...
  const offline = () => {
    say(`The assistant is not available right now. Ring ${PHONE} or email ${EMAIL} and a person will help.`);
    actions.innerHTML = `<a class="btn btn-primary btn-sm" href="${PHONE_HREF}">Ring ${PHONE}</a><a class="btn btn-ghost btn-sm" href="mailto:${EMAIL}">Email us</a>`;
  };
  ...
  async function send(text, photos = []) {
    ...
    try {
      ...
      if (res.done) {
        chips([["Report something else", () => startRepair()], ["That is all, thanks", () => { say("Thank you. The team will be in touch."); actions.innerHTML = ""; }]]);
      } else {
        ask("Type your reply", send, { multiline: true, photos: state.mode === "repair" || state.mode === "repair-unverified" });
      }
    } catch (e) {
      stop();
      if (e.status === 404 || e.status === 405 || e.status === 0 || !e.status) return offline();
      ...
      ask("Type your reply", send, { multiline: true, photos: true });
    }
  }
  ...
  async function sendCode(channel) {
    ...
    try {
      ...
      stop();
      state.verifyToken = res.token; save();
  ...
  function askCode(attempt = 1) {
    ask("Six-digit code", async (code) => {
      ...
      try {
        ...
        else if (e.status === 410) { say("That code has expired."); chips([["Send a new code", () => sendCode("email")], ["Continue without verification", () => unverified("Code expired.")]]); }
        else { say("I could not verify that code."); chips([["Send a new code", () => sendCode("email")], ["Continue without verification", () => unverified("Verification failed.")]]); }
      }
  ...
  /* --- Entry points --- */
  function start() {
    setStatus("Repairs, tenancies and enquiries");
