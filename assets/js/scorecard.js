...
(() => {
  ...
  function render(d) {
    ...
    $("#sc-progress").innerHTML = CH.progress(d.score, [{ name: "Bronze", min: 50 }, { name: "Silver", min: 70 }, { name: "Gold", min: 85 }, { name: "Platinum", min: 95 }]);
    const partColours = [C.gold, C.ink, C.moss, C.slate];
    ...
    const word = (ch) => ch === "O" ? "On time" : ch === "L" ? "Late" : ch === "P" ? "Part paid" : ch === "M" ? "Missed" : "Not due";
    const height = (ch) => ch === "O" ? 100 : ch === "L" ? 70 : ch === "P" ? 50 : ch === "M" ? 8 : 0;
    const colour = (ch) => ch === "O" ? C.moss : ch === "L" ? C.gold : ch === "P" ? C.goldSoft : ch === "M" ? C.accent : C.paper2;
    $("#sc-bars").innerHTML = CH.bars([...hist].map((ch, i) => ({ label: MONTHS[(start + i) % 12], values: [{ v: height(ch), color: colour(ch), title: `${MONTHS[(start + i) % 12]}: ${word(ch)}` }] })), { max: 100, cls: "bars-months" });
    $("#sc-months").innerHTML = CH.legend([{ color: C.moss, label: "On time", text: [...hist].filter((c) => c === "O").length }, { color: C.gold, label: "Late", text: [...hist].filter((c) => c === "L").length }, { color: C.accent, label: "Missed", text: [...hist].filter((c) => c === "M").length }]);
    ...
    const jobs = d.jobs || [];
    $("#sc-jobs").innerHTML = jobs.length ? `<div class="chart-card chart-wide"><div class="chart-head"><h3>Your repairs</h3><span class="muted">${jobs.filter((j) => !j.closed).length} open</span></div><div class="jobs"><ul>${jobs.map((j) => `<li><span class="job-status job-${j.closed ? "closed" : "open"}">${CH.esc(j.status || (j.closed ? "Closed" : "Open"))}</span><strong>${CH.esc(j.title)}</strong><small>${CH.esc(j.ticket ? `Ticket ${j.ticket} · ` : "")}${CH.esc(j.created || "")}</small></li>`).join("")}</ul></div></div>` : "";
