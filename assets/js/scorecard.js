...
(() => {
  ...
  function render(d) {
    ...
    $("#sc-parts").innerHTML = d.parts.map((p) => `<div class="score-part"><div class="score-part-head"><span>${p.label}</span><strong>${p.score} / ${p.max}</strong></div><div class="score-bar"><i style="width:${(p.score / p.max) * 100}%"></i></div><small>${p.detail}</small></div>`).join("");
