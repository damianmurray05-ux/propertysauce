...
- OpenRent applicants cannot be answered by email at all. OpenRent's relay only accepts replies from the address registered on the account, which has been Lettings@propertysauce.co since 31 August on Claude's earlier advice, and admin@ cannot send as it. As of 19 September that had blocked replies to applicants for nineteen days; three viewing requests on Flat 42 alone in twelve hours went unanswered, on a flat that had already been let and whose advert was still live.
...
|---|---|---|---|---|
| admin@propertysauce.co, with info@, Lettings@, invoices@, enquiries@ landing in it | `gws-propertysauce` | yes | admin@ only | All Property Sauce replies, including OpenRent once the account is back on admin@ (section 10, question 2) |
...
2. **OpenRent registered address.** On 31 August 2026 the OpenRent account (3944818, Usman) was changed to Lettings@propertysauce.co on Claude's earlier advice. That was wrong: OpenRent only accepts replies to applicants from the registered address, and admin@ cannot send as Lettings@. OpenRent's own error email (2, 6 and 13 September) says "that address is not registered with OpenRent". Damian's decision, 20 September 2026: change the OpenRent account email back to admin@propertysauce.co. Usman asked by Slack direct message the same day. Once done, applicant replies go from admin@ like everything else, and Lettings@ needs nothing. Until done, every OpenRent applicant is posted to the viewings channel for the person with the OpenRent login.
...
7. Usman changes the OpenRent account email back to admin@propertysauce.co (question 2), then Claude tests a reply to the newest viewing request.
