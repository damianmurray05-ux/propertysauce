...
Applicants are kept out of Zoho CRM. Every enquiry lives in the Google Sheet "Applicants database" (Drive, My Drive/Claude/01 Property Sauce/Lettings) and is emailed from lettings@propertysauce.co. A Tenant record is created in Zoho only when the agreement is signed. Steps 4 and 5 and section 3 below are written to match; anything else in this file that says an applicant has a Zoho record is superseded.
...
**Zoho CRM, Tenant (Contacts) record.** Email templates for this operation are in the folder "Ops 01 Lettings" (section 9). The `Status` picklist (read from the layout editor on 20 September 2026) is Application Stage, Let Agreed, Tenanted, Arrears, Possession Proceedings, Court, Vacated, Maintenance Only, Archived, so the applicant status already exists as **Application Stage** and nothing is added. Applicants are NOT created here (Damian, 20 September 2026): the Tenant record is created only at agreement signing. Application Stage stays in the picklist unused; the Applicants database sheet in Drive holds every enquiry until then.
...
- **When the Tenant record is created.** Settled by Damian on 20 September 2026: only when the applicant becomes a tenant, at agreement signing (Ops 02 creates the record with the signed agreement attached and copies the applicant row into it). Applicants stay in the Drive sheet; the Zoho fields Applicant_Area, Enquiry_Source and Last_Contacted are unused and may be removed. Zoho held no applicant records on 20 September, so nothing was deleted.
