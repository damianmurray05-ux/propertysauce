...
The matching step is built: `scripts/ops/rent-match.mjs` (23 September 2026). It reads only; nothing is written to Zoho Books or Zoho CRM and nothing is sent. The rules it applies are the section 4 rule, spelled out in the header of the file: a line is CERTAIN only when (a) the description carries an open invoice number that belongs to one customer, (b) it carries the tenancy's payment reference as a whole word and only one tenancy with open invoices uses that reference, or (c) the payer name matches one tenant (surname plus first name or initial) and the amount equals that tenancy's rent or an open invoice, and no other tenancy matches. Everything else is UNCERTAIN, with the best one or two candidates and the reason, and lines that are plainly not rent (transfers between our own accounts and companies, card refunds and cashback, holding deposits, insurance and service charge contributions, loans, fees) go to a NOT-RENT list.

**Daily at 07:30, seven days a week** (rent lands overnight; the timetable in 5b counts calendar days):

1. Run `node --env-file=.env scripts/ops/rent-match.mjs`. It pulls every uncategorised money-in line on the Property Sauce bank feeds (Virgin Money rent, Deposit and Fee accounts and the Lloyds business account; the inactive "Property Sauce Fees" account is skipped), builds the tenancy table from Books customers and CRM Tenant records, and writes `certain.csv`, `uncertain.csv` and `not-rent.csv` to `docs/ops/rent-matching/` and to Drive, "Claude/01 Property Sauce/Accounts/Rent matching". Descriptions are copied as the bank sends them with any run of eight or more digits masked, so no account number reaches the files (README rule 7).
2. **Certain matches are recorded**: the payment is recorded in Books against the invoice named in the file (or on the customer's account when the payment predates every open invoice, which Books requires), the Tenant record gets last payment date, amount and arrears balance, and a tenant who paid with the wrong reference gets template R0 (5a step 3). Recording is switched off until Damian has reviewed `certain.csv` from the first runs: `--apply` refuses with "recording payments is not enabled until Damian has reviewed certain.csv". Until it is enabled, the certain list is posted for a person to record by hand.
3. **Uncertain lines go to a person in Slack #accounts-rent** (C0BSZMD5REF), one message per run: date, account, amount, description, the candidates and the reason, oldest first, with `uncertain.csv` attached. The person allocates in Books or replies in the thread; Claude never guesses and never allocates an uncertain line. The not-rent count and file go in the same message for Ops 14 to categorise.
4. **Arrears chasing follows the 5b timetable**: for every tenancy with an open invoice past its due date, work out the day number (due date is day 0) and run that day's action on days 1, 3, 7, 10, 14 and 21, within the limits in 5e (a hardship, vulnerability or dispute flag stops the timetable; no contact before 8am or after 8pm; one call attempt a day; guarantor copies only where a deed is on file). Every action is written to the Tenant record with the next action date. Day 21 files go to Marchbank & Vale as 5c says, except flagged files, which go to Damian.
5. Post a one-line summary to #accounts-rent: lines read, certain / uncertain / not-rent counts and values, reminders and letters sent, files handed over.

**Weekly, Monday 08:00**: arrears report by landlord (tenancy, balance, stage, next action date) to #accounts-rent and to Damian, feeding the Ops 10 statements.

**Backlog**: the first dry run on 23 September 2026 read 316 uncategorised money-in lines going back to 2019: 6 certain (£2,792.68), 85 uncertain (£33,377.75) and 225 not rent (£189,869.50, almost all transfers between our own accounts and companies). The backlog is worked by a person from the three files; the routine only adds each day's new lines. Two things the run showed that need fixing in Zoho before `--apply` is enabled: 27 current CRM tenants have no Books customer (so their payments can never be certain), and the Lancaster House flat references (NE236UNFLATnn) outlive a tenancy, so a re-let flat needs a new Books customer before the reference can be trusted again.
