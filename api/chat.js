...
const MODES = new Set(["repair", "repair-unverified", "tenancy", "general"]);
...
export async function POST(request) {
  ...
  const mode = MODES.has(body.mode) ? body.mode : "general";
  ...
  let useFallbacks = process.env.CHAT_FALLBACKS !== "0";
  const create = async (params) => {
    if (useFallbacks) {
      try {
        ...
        useFallbacks = false; // parameter not accepted here: carry on without it
      }
    }
