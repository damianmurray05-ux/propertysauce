...
export async function POST(request) {
  ...
  if (body.action === "lookup") {
    ...
    const channels = { email: (emailConfigured() || devEcho()) && t.email ? maskEmail(t.email) : null, phone: smsConfigured() && t.phone ? maskPhone(t.phone) : null };
  ...
  if (body.action === "start") {
    ...
    if (devEcho() && !emailConfigured() && !smsConfigured()) sent = { ok: true };
      ...
      sent = await sendSms({ to: t.phone, text: `Property Sauce: your verification code is ${code}. It expires in 10 minutes. If you did not request this, ignore it.` });
    } else if (t.email) {
      sent = await sendEmail({ to: t.email, subject: `Your Property Sauce code: ${code}`, text: `Your one-time code is ${code}.\n\nEnter it in the assistant on propertysauce.co to continue. It expires in 10 minutes. If you did not request this, ignore this email.\n\nProperty Sauce` });
