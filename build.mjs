// Property Sauce static site build.
// `node build.mjs` writes the finished site to dist/. Zero build dependencies.
...
for (const file of readdirSync(pagesDir).filter((f) => f.endsWith(".html"))) {
  ...
  let bodyHtml = m[2];
  // Optional per-page JSON-LD block: <script type="application/ld+json" data-page-ld>...</script>
  const ld = bodyHtml.match(/<script type="application\/ld\+json" data-page-ld>([\s\S]*?)<\/script>/);
  if (ld) {
  ...
  if (meta.noindex !== "true") urls.push({ path, priority: meta.priority || (path === "/" ? "1.0" : "0.7") });
}
...
writeFileSync(
  ...
  `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n` +
    urls.map((u) => `  <url><loc>${site.url}${u.path}</loc><lastmod>${today}</lastmod><priority>${u.priority}</priority></url>`).join("\n") +
    `\n</urlset>\n`
);
writeFileSync(join(dist, "robots.txt"), `User-agent: *\nAllow: /\nDisallow: /api/\nSitemap: ${site.url}/sitemap.xml\n`);
writeFileSync(
  join(dist, "site.webmanifest"),
  JSON.stringify({ name: site.name, short_name: "Property Sauce", start_url: "/", display: "browser", background_color: "#f4f5f1", theme_color: "#0f2a22", icons: [{ src: "/brand/mark-512.png", sizes: "512x512", type: "image/png" }] }, null, 2)
);
